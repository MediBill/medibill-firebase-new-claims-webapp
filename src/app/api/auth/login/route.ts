...
export async function POST(request: NextRequest) {
  ...
  // console.log(`[API Route] Attempting login to external API: ${LOGIN_ENDPOINT} with email: ${APP_EMAIL}`); // Removed for prod
  ...
  try {
    ...
    if (!externalApiResponse.ok) {
        // Log the raw response text for debugging non-OK responses, but maybe not the full text in prod for sensitive data
        console.warn(`[API Route] External API login response text (Status: ${externalApiResponse.status}): ${responseText.substring(0, 200)}...`);
    }
    ...
    try {
      ...
      // console.error(`[API Route] Raw response text was: ${responseText}`); // Potentially too verbose for prod
    ...
    if (responseData.status === 'success' && responseData.token) {
      ...
      // console.log('[API Route] External API login successful.'); // Removed for prod
